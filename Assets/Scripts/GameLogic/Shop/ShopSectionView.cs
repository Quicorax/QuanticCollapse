...
namespace QuanticCollapse
{
    public class ShopSectionView : MonoBehaviour
    {
        ...
        public void InitProductSection(string productName, List<ShopElementModel> ShopElements,
            Action<ShopElementModel> purchaseAction, Transform sectionParent)
        {
            productHeader.text = _localization.Localize(productName);
            _purchaseAction = purchaseAction;
            _sectionParent = sectionParent;

            foreach (var shopElements in ShopElements.Where(shopElements => shopElements.Product.Id == productName))
            {
                _addressables.LoadAddrsOfComponent<ShopElementView>("SectionProduct",
                    _elementParent, x => x.InitProduct(shopElements, BuyProduct).ManageTaskException());
        ...
        private void BuyProduct(ShopElementModel transactionData)
        {
            _transactionOnSight = transactionData;
            PurchaseInGamePopUp(transactionData);
        }

        private void PurchaseInGamePopUp(ShopElementModel transactionData)
        {
